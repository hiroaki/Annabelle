...
describe LocaleUtils do
  ...
  describe '.determine_locale' do
    ...
    it 'returns the param locale if valid' do
      params = { locale: 'ja' }
      expect(LocaleUtils.determine_locale(params, request, user)).to eq('ja')
    end
    ...
    it 'returns header locale if param and user are invalid' do
      ...
      expect(LocaleUtils.determine_locale(params, request, user2)).to eq('ja')
    end
    ...
    it 'returns default locale if nothing matches' do
      ...
      expect(LocaleUtils.determine_locale(params, req, user2)).to eq(LocaleConfiguration.default_locale.to_s)
    end
  end

  describe '.valid_locale?' do
    it 'returns true for supported locale' do
      expect(LocaleUtils.valid_locale?('ja')).to be true
    end
    it 'returns false for unsupported locale' do
      expect(LocaleUtils.valid_locale?('fr')).to be false
    end
  end
end
