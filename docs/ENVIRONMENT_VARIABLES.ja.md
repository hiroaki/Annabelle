...
リクエストのサイズ制限の第一防衛線はプロキシサーバです。

一方で `MAX_REQUEST_BODY` は、アプリケーション側でも二重チェックとして利用します。

- ブラウザでは、可能な場合に送信前エラーを表示するための事前チェックに使います
- Rails アプリケーションでは、過大な本文や、Blob 作成前の過大なメッセージ投稿を拒否するために使います

この値はバイト単位で設定し、`DEPLOY_PROXY_BUFFERING_MAX_REQUEST_BODY` と揃えてください。通常運用ではプロキシが主たる制限役であり、アプリケーション側のチェックは、クライアント側チェックの回避や設定ずれに備える安全策です。

```
MAX_REQUEST_BODY=10485760
```

意図的に別の値を使う場合でも、アプリケーション側がプロキシより大きい値を許容しないよう、`MAX_REQUEST_BODY` は `DEPLOY_PROXY_BUFFERING_MAX_REQUEST_BODY` 以下にしてください。
...
`DEPLOY_PROXY_BUFFERING_MAX_REQUEST_BODY` は、プロキシが制限するリクエストの最大サイズをバイト単位で設定します。これを超える場合、プロキシは HTTP ステータス 413 を返し、リクエストはアプリケーションへ到達しません。`MAX_REQUEST_BODY` もこの値と揃え、クライアント側・アプリケーション側・プロキシ側の挙動差をできるだけ小さくしてください。
...
Annabelle は、既定では別 worker プロセスを必須にしない軽量な単一サーバ構成でも動くようにしています。

ただし、`perform_later` や Active Storage の `purge_later` を使う処理は存在します。そのため既定の軽量構成では、これらのジョブは小規模用途では許容できる一方で、再起動や停止のタイミングによっては遅延したり失われたりし得る、という前提で扱ってください。

単一サーバ構成のまま Solid Queue を web プロセスと同居させたい場合は、次の環境変数を有効にすると Puma が Solid Queue supervisor を起動できます。

```
SOLID_QUEUE_IN_PUMA=1
```

対応する Puma 設定は `config/puma.rb` を参照してください。
